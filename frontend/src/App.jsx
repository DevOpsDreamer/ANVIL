...
export default function App() {
  ...
  const {
    ...
    result, error, retryInfo, start, reset,
  } = useScan();
  ...
  const [repoUrl, setRepoUrl] = [
    // We keep repoUrl in local state so RepoInput can display it
    null, (url) => {} // placeholder — we derive from scan
  ];

  // We need repoUrl for Terminal — derive it from result or from the last started scan
  const displayRepoUrl = result?.repo_url || null;
            ...
            <div className={styles.stepContent}>
              ...
              {(isScanning || isComplete) && (
                <div className={styles.scanningBanner}>
                  ...
                  <span className={styles.scanningRepo}>{result?.repo_url || '…'}</span>
            ...
            <div className={styles.stepsGrid}>
              {[
                { n:'01', color:'#00b4ff', title:'GitHub OAuth',
                  desc:'Authenticate with GitHub so ANVIL can clone your repo and open a Pull Request with the fix using the repo scope.' },
